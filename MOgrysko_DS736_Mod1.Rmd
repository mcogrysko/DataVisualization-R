---
title: "2022 FIFA World Cup - Notable Characteristics of Winning Teams"
author: "Mike Ogrysko"
...
This report examines individual game data from the 2022 FIFA World Cup, played in Qatar from November 20, 2022 to December 18, 2022. The visualizations in this report focus on the teams that advanced and had success in different rounds during the World Cup. For some background, there were 32 teams that competed in 64 matches to determine the champion. In the first round, each team was added to one of eight groups in which it plays three games. Sixteen teams advance from the first round and the rest of the play is single-elimination. The final four teams play a total of seven games - Argentina bested France in the final and Croatia beat Morocco in the 3rd place game. 

## Dataset

The data used for this report is the FIFA World Cup 2022: Complete Dataset found on <a href="https://www.kaggle.com/datasets/die9origephit/fifa-world-cup-2022-complete-dataset" target="_blank">Kaggle</a>. Each record in this dataset represents an individual game in the 2022 World Cup. The data in each record includes individual team metrics achieved during the game. A record includes a Team 1 and a Team 2, as well as data like possession percentage, goals scored, attempts on goal, assists, corner kicks, and fouls for each team. In total, this dataset includes 64 records (one for each game in the World Cup) and 88 features. 

## Findings {.tabset .tabset-fade .tabset-pills}

Given the individual game data statistics for each team provided in each game record, it seemed interesting to examine the dataset to determine if there were any notable characteristics among the teams that advanced through the World Cup. The following visualizations are discussed in the tabs below:
<br><br>
<li>Tab 1 - 1st Round Attempts Count by Quarterfinals Teams (Horizontal Stacked Bar Chart)</li>
<li>Tab 2 - Final 4 Team Fouls by Game (Multiple Line Charts)</li>
<li>Tab 3 - Forced Turnovers by Game by Team - Final 4 Teams (Multiple Bar Charts)</li>
<li>Tab 4 - Passes Completed by Team by Game (Heatmap)</li>
<li>Tab 5 - Time of Possession in Argentina's Games (Trellis Pie Charts)</li>
<br>

### Tab 1 

In this visualization, I created a horizontal stacked bar chart to show the different types of attempts on goal and the First-Round quantities by teams that advanced to the Quarterfinals. Here, we can see that teams that advanced to the quarterfinals tended to make the most attempts inside the penalty area. In addition, we see that there were nearly equal attempts on- and off-target. In terms of teams, we can see that Argentina, France, and Croatia had the most attempts of the group of eight teams, while Portugal, England, and the Netherlands had the least. Why might some teams have had more attempts than others? 
...
p1mod1 <- ggplot(team_attempts_df, aes(x = reorder(Description, n, sum), y = n, fill = team)) +
...
In this visualization, I created a multi-line chart to show the team fouls incurred by teams advancing to the semifinals in each of their seven games.  There are some interesting trends to note for some of the teams. After starting off Game 1 with 17 fouls, Morocco’s team fouls experienced a downward trend thereafter. For the most part, France’s team fouls increased with each game (except for Game 6). Croatia and Argentina’s team fouls fluctuated from game-to-game. Interestingly, these teams had the highest single game totals of the four teams – Croatia had 22 in Game 5 and Argentina had 27 in Game 7 (the final). France had their highest team foul total in the final. 
...
p2mod1 <- ggplot(all_fouls, aes(x=game_order, y=n, group=team)) + 
  geom_line(aes(color=team), linewidth=3) + 
...
Here, the multiple bar charts show the number of forced turnovers by the teams that advanced to the semifinals in each of their games. For the most part, Croatia, France, and Morocco had fluctuating forced turnovers. Argentina appears to have had the fewest turnovers (494) and kept their turnovers relatively even throughout the World Cup. They had the most turnovers in the final with 87. France had the most single-game turnovers of the four teams with 104 in the final. Interestingly, finals teams Argentina and France had more forced turnovers than their opponents in their Game 6 victories. 
...
This heatmap shows the completed passes by semifinals teams in each of their seven games. Argentina had the most single-game passes completed with 814 in Game 3 (a win). Morocco had the fewest single-game passes completed with 171 in Game 5 (also a win). Interestingly, Argentina and France had their fewest single-game passes completed with 344 and 307, respectively, in Game 6 – both winning despite the low totals. What might be the reason behind their low completion of passes in these games? Looking at the final, Argentina had more completed passes in victory, 544, than France, 419. Overall, does there appear to be a correlation between passes completed and the end result? 
...
This visualization shows the time of possession in each of World Cup champion Argentina’s seven games. In each pie chart, we can see Argentina’s time of possession (in blue), their opponent’s time of possession (color varies), and the contested time of possession (in green). Interestingly, Argentina lost Game 1, but had their highest time of possession with 64%. What does this suggest? Potentially, this could mean that Argentina might have controlled the ball throughout the game but was unable to score. In Game 5, Argentina ended regulation time with a tie (and won in a shootout). In the pie chart 5, we can see that they split the time of possession with the Netherlands. In their win in Game 6, we can see that Argentina had their lowest time of possession with 34%. Why might this be? Is it a byproduct of their early first half lead? 
...
p5mod1 <- plot_ly(textposition="inside", labels = ~team, values = ~possession, hovertemplate = paste('Team: %{label}<br>Possession: %{value}%<br>')) %>%
  add_pie(data=all_arg[all_arg$Game == 1,], 
          name="1", title="Game 1", domain=list(row=0, column=0)) %>%
...
This is a brief look at the data from the World Cup and is not a deep enough dive to truly identify all of the relevant and common characteristics of the winning teams. World Cup winner Argentina had high numbers of attempts in the 1st Round, the fewest forced turnovers among the final 4 teams, the most single game passes completed, as well as the most completed passes in the final, and controlled the time of possession in most of their games. Focusing on the final, Argentina had fewer forced turnovers than France, more completed passes, and controlled the time of possession. These favorable metrics align with their resulting World Cup victory. However, there were anomalies in their path to victory - losing Game 1 despite controlling the time of possession, winning Game 6 despite only controlling the time of possession for 34% of the game, and out-fouling France (and beating them) in the final. It is these anomalies that could be worth some additional analysis. When does time of possession matter? How do team fouls relate to a team's success? What other factors contribute to team victories and losses? This dataset had 88 features, so there are many more insights that could be found from this data alone. 
